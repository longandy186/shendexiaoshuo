 ...
 *
 * 此文件替代 database-storage.ts（基于 localStorage 的旧版本）
 */
...
export async function dbGetWorldSettingsByNovelId(novelId: string): Promise<DBWorldSetting[]> {
  try {
    ...
    if (error) {
      console.error('[SupabaseSettings] 获取世界观设定失败:', error);
      return [];
    }

    return (data || []).map(transformWorldSetting);
  } catch (error) {
    console.error('[SupabaseSettings] 获取世界观设定异常:', error);
    return [];
  }
}
...
export async function dbCreateWorldSetting(
  ...
  try {
    ...
    if (error || !data) {
      console.error('[SupabaseSettings] 创建世界观设定失败:', error);
      return null;
    }

    return transformWorldSetting(data);
  } catch (error) {
    console.error('[SupabaseSettings] 创建世界观设定异常:', error);
    return null;
  }
}
...
export async function dbUpdateWorldSetting(
  ...
  try {
    ...
    if (error || !data) {
      console.error('[SupabaseSettings] 更新世界观设定失败:', error);
      return null;
    }

    return transformWorldSetting(data);
  } catch (error) {
    console.error('[SupabaseSettings] 更新世界观设定异常:', error);
    return null;
  }
}
...
export async function dbDeleteWorldSetting(id: string): Promise<boolean> {
  try {
    ...
    if (error) {
      console.error('[SupabaseSettings] 删除世界观设定失败:', error);
      return false;
    }

    return true;
  } catch (error) {
    console.error('[SupabaseSettings] 删除世界观设定异常:', error);
    return false;
  }
}
...
export async function dbGetWorldSettingsByCategory(novelId: string, category: string): Promise<DBWorldSetting[]> {
  const allSettings = await dbGetWorldSettingsByNovelId(novelId);
...
export async function dbGetCharactersByNovelId(novelId: string): Promise<DBCharacter[]> {
  try {
    ...
    if (error) {
      console.error('[SupabaseSettings] 获取角色设定失败:', error);
      return [];
    }

    return (data || []).map(transformCharacter);
  } catch (error) {
    console.error('[SupabaseSettings] 获取角色设定异常:', error);
    return [];
  }
}
...
export async function dbCreateCharacter(
  ...
  try {
    ...
    if (error || !data) {
      console.error('[SupabaseSettings] 创建角色设定失败:', error);
      return null;
    }

    return transformCharacter(data);
  } catch (error) {
    console.error('[SupabaseSettings] 创建角色设定异常:', error);
    return null;
  }
}
...
export async function dbUpdateCharacter(
  ...
  try {
    ...
    if (error || !data) {
      console.error('[SupabaseSettings] 更新角色设定失败:', error);
      return null;
    }

    return transformCharacter(data);
  } catch (error) {
    console.error('[SupabaseSettings] 更新角色设定异常:', error);
    return null;
  }
}
...
export async function dbDeleteCharacter(id: string): Promise<boolean> {
  try {
    ...
    if (error) {
      console.error('[SupabaseSettings] 删除角色设定失败:', error);
      return false;
    }

    return true;
  } catch (error) {
    console.error('[SupabaseSettings] 删除角色设定异常:', error);
    return false;
  }
}
...
export async function dbGetNovelById(id: string): Promise<DBNovel | null> {
  try {
    ...
    console.error('[SupabaseSettings] 获取小说失败:', error);
    return null;
  }
}
...
export async function dbCreateNovel(
  ...
  try {
    ...
    if (error || !data) {
      console.error('[SupabaseSettings] 创建小说失败:', error);
      return null;
    }
    ...
    console.error('[SupabaseSettings] 创建小说异常:', error);
    return null;
  }
}
...
export async function dbClearDatabase(): Promise<void> {
  try {
    ...
    console.error('[SupabaseSettings] 清空数据库失败:', error);
  }
}
