...
/**
 * GET /api/novels/[id]/revisions/[recordId] - 获取特定推敲记录
 */
export async function GET(
  ...
  try {
    ...
    if (error) {
      console.error('[GET /revisions/:recordId] 查询错误:', error);
      return errorResponse('记录不存在', 404);
    }

    return successResponse(data, '获取推敲记录成功');
  } catch (error: any) {
    console.error('[GET /revisions/:recordId] 获取推敲记录失败:', error);
    return errorResponse('获取推敲记录失败', 500);
  }
}

/**
 * DELETE /api/novels/[id]/revisions/[recordId] - 删除推敲记录
 */
export async function DELETE(
  ...
  try {
    ...
    if (error) {
      console.error('[DELETE /revisions/:recordId] 删除错误:', error);
      return errorResponse('删除失败', 500);
    }

    return successResponse({ deleted: true }, '删除推敲记录成功');
  } catch (error: any) {
    console.error('[DELETE /revisions/:recordId] 删除推敲记录失败:', error);
    return errorResponse('删除推敲记录失败', 500);
  }
}
